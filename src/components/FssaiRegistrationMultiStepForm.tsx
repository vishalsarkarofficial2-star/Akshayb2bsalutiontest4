...
interface FssaiRegistrationMultiStepFormProps {
...
const FSSAI_PACKAGES = [
  { id: '1year-basic', name: '1 Year FSSAI Basic Registration', price: '₹899', desc: 'Turnover ≤ ₹12 Lakhs (All-inclusive)' },
  { id: '3year-basic', name: '3 Year FSSAI Basic Registration', price: '₹1,499', desc: 'Save on multi-year renewals' },
  { id: '5year-basic', name: '5 Year FSSAI Basic Registration', price: '₹2,199', desc: 'Max validity 5-year tenure' },
  { id: 'state-license', name: 'FSSAI State License Tier', price: '₹2,499 + Govt Fee', desc: 'Turnover ₹12L - ₹20Cr' },
  { id: 'central-license', name: 'FSSAI Central License Tier', price: '₹3,999 + Govt Fee', desc: 'Turnover > ₹20Cr / Importers' }
];

export const FssaiRegistrationMultiStepForm: React.FC<FssaiRegistrationMultiStepFormProps> = ({ onSuccess, onApply }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    panNumber: '',
    packageTier: '1 Year FSSAI Basic Registration - ₹899',
    captchaAnswer: '',
    fullName: '',
    email: '',
    phone: ''
  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      if (!formData.state) {
        setError('Please select your state or UT.');
        return;
      }
    } else if (step === 2) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSubmitted(true);
      if (onApply) onApply('FSSAI Registration Lead Submitted');
  ...
  if (isSubmitted) {
    return (
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-8 text-center relative overflow-hidden">
        ...
        <h3 className="text-2xl font-extrabold text-slate-900 mb-2">FSSAI Application Initiated!</h3>
        <p className="text-slate-600 text-sm mb-6 leading-relaxed">
          Thank you, <span className="font-bold text-slate-900">{formData.fullName}</span>. Your FSSAI registration request for <span className="font-bold text-slate-900">{formData.packageTier}</span> in <span className="font-bold text-slate-900">{formData.state}</span> has been received. Our FoSCoS legal consultant will contact you within 15 minutes.
        </p>
        <button
          onClick={() => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 relative overflow-hidden">
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <span className="px-2.5 py-1 bg-orange-50 text-[#FF6B00] text-[10px] sm:text-xs font-black uppercase tracking-wider rounded-md border border-orange-100 flex items-center gap-1">
            ⚡ Fast-Track FSSAI FoSCoS Registration
          </span>
        ...
        <div className="mb-6">
          ...
          <h3 className="text-xl sm:text-2xl font-black text-slate-900">
            ...
            {step === 3 && 'PAN & Package Selection'}
          </h3>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            {step === 1 && 'Choose the primary state or Union Territory for your food business jurisdiction.'}
            {step === 2 && 'Select your legal business structure and primary food business activity.'}
            {step === 3 && 'Enter PAN details, choose your FSSAI registration package tier, and solve security captcha.'}
          </p>
        </div>
        ...
        <form onSubmit={step === 3 ? handleSubmit : (e) => { e.preventDefault(); handleNext(); }}>
          ...
          {/* STEP 3: PAN Number field, Package dropdown, Captcha */}
          {step === 3 && (
            <div className="space-y-4 animate-in fade-in duration-300">
            ...
            {step < 3 ? (
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <span>Submit FSSAI Application</span>
                    <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>
      ...
      <div className="bg-slate-50 px-6 py-3 flex items-center justify-center gap-2 text-xs font-bold text-emerald-700 border-t border-slate-100">
        ...
        <span>100% Confidentiality & Government FoSCoS Portal Verified</span>
      </div>
    </div>
  );
};
